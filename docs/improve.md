# ERL-DRG 流程阶段性结果分析与改进建议

本文档用于分析主流程中 **4.5 步后收集到的 `tau/R/rho`** 与 **完整进化流程 5.6 步后收集到的 `tau/R/rho`** 的对比结果。
...
1. 读取数据集 `Prompts & Labels`，其中 Labels 用于最终计算 ORM 分数 `rho`。
2. 随机初始化扩散模型。
3. 随机初始化 `M` 个 Qwen 模型，形成初始种群。
4. 对每个 Qwen 个体执行：
   - 用该 Qwen 对 `N` 个 Prompts 生成回答；
   - 收集 PPO 训练数据 `[Context, Response, Reward(空)]`；
   - 收集 hidden states，记为 `tau`；
   - 用扩散模型根据 `tau` 生成奖励序列 `R`；
   - 将 `R` 填入 PPO 数据中的 reward；
   - 使用这份数据对当前 Qwen 进行 PPO 训练；
   - 再次推理生成回答，并计算 ORM 分数 `rho`。

因此，4.5 步后得到的是：

\[
(\tau_i, R_i, \rho_i, \theta_i), \quad i=1,2,\dots,M
\]

其中：

- `tau_i` 表示第 `i` 个 Qwen 个体生成时的 hidden states；
- `R_i` 表示扩散模型给该轨迹生成的奖励序列；
- `rho_i` 表示该个体在真实评价标准，也就是 ORM 上的分数；
- `theta_i` 表示该 Qwen 个体的参数。

### 1.2 进化迭代阶段

完整流程中，算法进一步执行 `K` 次进化迭代：

1. 用当前种群中的 `tau/R/rho` 训练扩散模型；
2. 根据 `rho` 选择 Top 精英个体；
3. 对精英个体进行基于扩散模型的奖励变异；
4. 用变异奖励 `R'` 再训练临时 Qwen；
5. 用新 Qwen 重新生成轨迹，得到新的 `tau_new`；
6. 再用扩散模型生成标准奖励 `R_new`；
7. 再次 PPO 训练，得到新个体；
8. 计算新个体的 ORM 分数 `rho_new`；
9. 将新个体加入种群，并根据 `rho` 保留 Top `M` 个个体。

理论上，完整流程跑完以后应该看到：

\[
\rho_{final} > \rho_{step4}
\]

也就是说，完整进化后的种群 ORM 分数应该整体上升。

---

## 2. `rho` 对比分析：完整进化后几乎没有提升
...
如果该个体 `tau` 很大，但 `rho` 不高，说明它不是有效进化，而是训练漂移。
...
当前只有箱线图，还不能完全判断 `R/tau` 与 `rho` 的关系。
...
如果算法有效，至少应该看到某种趋势，例如：

\[
R_{mean} \uparrow \Rightarrow \rho \uparrow
\]

或者：

\[
某种 \tau \text{ 分布模式} \Rightarrow \rho \text{ 更高}
\]

如果这些图完全没有相关性，就说明扩散奖励和最终 ORM 表现之间还没有建立联系。
